...
- 可选参数未传 → 使用安全默认值
  - 对象属性：`obj?.prop ?? default`（null/undefined 时才降级）
  - 数值/NaN：`parseInt(x) || default`（🛑 `??` 不捕获 NaN，parseInt(undefined) 返回 NaN）
...
### 11.6 环境变量白名单
代码中通过 `process.env.X` 引用的变量分两类处理：
| 类别 | 变量示例 | 处理方式 |
|------|---------|---------|
| 基础设施变量 | `PORT`, `DB_*`, `JWT_*`, `ENCRYPTION_KEY`, `LOG_LEVEL`, `REDIS_URL` | integrator 自动补齐到 4 个 env 文件 |
| 第三方服务变量 | `SMS_*`, `WECHAT_PAY_*`, `ALIPAY_*`, `WECHAT_APP_*`, `OSS_*`, `ELASTICSEARCH_HOST` | 只有代码实际引用了才创建（参考场景树 §2 的 `基础设施` 列），不预创建未使用的变量 |

🛑 **env 文件中的值不带引号**（`PORT=3000` 不是 `PORT="3000"`）。
🛑 **4 个 env 文件的变量集必须完全一致**（`.env` / `.env.example` / `.env.development` / `.env.production`）。

---

## 十二、第三方服务双模式（Mock vs Real）

所有涉及第三方服务（短信/支付/OSS/邮件/推送）的代码必须同时支持 `*_MODE=mock` 和 `*_MODE=real` 两种运行模式。

### 12.1 环境变量约定

```
SMS_MODE=mock|real      # 短信服务
PAY_MODE=mock|real      # 支付服务
OSS_MODE=mock|real      # 对象存储
EMAIL_MODE=mock|real    # 邮件服务
```

默认值统一为 `mock`（`process.env.SMS_MODE || 'mock'`），开箱即用不依赖外部服务。

### 12.2 后端双模式实现

```javascript
// ✅ 模式分支在 service 层，对外接口统一
async function sendSmsCode(phone) {
  // ... 频率限制 + 验证码生成 ...
  if (SMS_MODE === 'mock') {
    // mock: 跳过真实发送，固定验证码 123456，打印日志
    logger.info(`[MOCK SMS] To: ${phone}, Code: 123456`);
  } else {
    // real: 调用第三方 API，失败抛异常
    await smsProvider.send({ phone, code, signName, templateCode });
  }
  return { success: true, mockCode: SMS_MODE === 'mock' ? code : undefined };
}
```

🛑 **铁律**：
- mock 模式下**跳过**第三方 API 调用，但**不跳过**业务逻辑（校验/限流/缓存/事务）
- mock 模式下返回固定值（短信验证码=123456，支付=成功），并在日志中标注 `[MOCK]`
- real 模式下第三方调用失败 → 必须 throw 明确错误码（如 `SMS_SEND_FAILED`）
- real 模式下**不得**静默降级为 mock 行为
- 返回值中 `mockCode` 只在 mock 模式出现，供前端展示提示用

### 12.3 前端双模式感知

```html
<!-- mock 模式：提示用户使用固定验证码 -->
<el-alert v-if="mockHint" type="info" :title="mockHint" show-icon closable />

<!-- 获取验证码按钮 — mock 模式倒计时仍需运行 -->
<el-button :disabled="cooldown > 0" @click="sendSms">
  {{ cooldown > 0 ? `${cooldown}s` : '获取验证码' }}
</el-button>
```

🛑 **铁律**：
- mock 模式下 API 返回含 `mockCode` 时，前端**必须**展示提示（如"模拟环境，验证码为 123456"）
- real 模式下不展示 mock 提示
- 前端通过检查响应中是否有 `mockCode` 字段来判断当前模式，**不读环境变量**

### 12.4 测试的 mock 期望

- 测试默认跑在 mock 模式（`.env` 中 `*_MODE=mock`）
- mock 模式下断言：不调 `crypto.randomInt`（用固定值）、不调外部 API
- 只在 nfr 层验证 real 模式的代码路径存在（静态检查 `SMS_MODE === 'real'` 分支存在即可）

---

## 十三、代码生成自检（Generation Self-Check）

生成任何文件后，必须逐项验证：

### 12.1 语法完整性
- 🛑 **花括号/括号/方括号必须配对**。文件末尾 `{` 数量 = `}` 数量。
- 🛑 **同一作用域禁止重复声明**。`const x = ...` 在同一模块/函数内只能出现一次。
- 🛑 **Vue template 每个开放标签必须有闭合标签**。`<div>` → `</div>`，自闭合 `<img />` 必须写 `/`。

### 12.2 模块导出规范
- 🛑 **Express 路由文件 `module.exports` 必须是单个 `router` 函数**，禁止导出 `{ r1, r2 }` 对象。
- 🛑 **前端 JS 使用 `export function/const` 命名导出**，禁止 CJS 语法。
- 🛑 **导出函数名必须与文件名/路由前缀一致**。

### 12.3 依赖完整性
- 🛑 **`require('xxx')` 或 `import 'xxx'` 引用新 npm 包时，确认 package.json 已有该依赖**。
- 🛑 **`require('./relative/path')` 的相对路径文件必须存在**。
