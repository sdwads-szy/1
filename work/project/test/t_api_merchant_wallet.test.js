...
describe('t_api_merchant_wallet', () => {
  ...
  const frontSrc = fs.readFileSync(path.join(workspace, 'src/api/merchant-wallet.js'), 'utf-8');

  // ==== 维度1: 路由匹配 ====
  describe('路由匹配', () => {
    it('GET / → handler: getWalletBalance', () => {
      expect(routeSrc).toContain('/');
      expect(ctrlSrc).toContain('getWalletBalance');
    });
  });

  // ==== 维度2: 字段一致 ====
  describe('字段一致', () => {
    it('controller 接收了预期字段: balance, frozenBalance, totalEarned', () => {
      ['balance', 'frozenBalance', 'totalEarned'].forEach(f => expect(ctrlSrc).toContain(f));
    });
  });
  ...
  describe('前端对齐', () => {
    it('前端 API 调用的端点与后端路由一致', () => {
      const endpoints = ['/merchant/wallet', 'getMerchantWallet'];
